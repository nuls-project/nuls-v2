 ...
 */
@Component
public class PbftRoundManager implements IRoundManager {
    /**
     * 添加轮次信息到轮次列表中
    ...
    public Map<String, List<String>> getAgentChangeInfo(Chain chain, BlockExtendsData lastExtendsData, BlockExtendsData currentExtendsData) {
        ...
        try {
            if (lastRoundIndex != -1) {
                ...
                if (lastRound == null) {
//todo                    lastRound = getRound(chain, lastExtendsData, false);
                }
            }
            currentRound = getRoundByIndex(chain, currentRoundIndex);
            if (currentRound == null) {
//todo                currentRound = getRound(chain, currentExtendsData, false);
            }
